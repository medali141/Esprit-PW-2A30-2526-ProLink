...
$allowed = ['id_participation', 'titre_event', 'nom', 'prenom', 'email', 'telephone', 'date_inscription', 'statut', 'id_event'];
$sort = (string) ($_GET['sort'] ?? 'date_inscription');
...
$rows = $pc->listeParticipation($sort, $dir);
...
class ProlinkPartPDF extends FPDF
{
    ...
    public function Footer(): void
    {
        ...
        $this->SetX(12);
        $w = 273;
        $this->Cell($w * 0.6, 4, prolink_pdf_s('ProLink  |  Back-office  |  Export des participations'), 0, 0, 'L');
        $this->Cell($w * 0.4, 4, prolink_pdf_s('Page ' . $this->PageNo() . ' / {nb}'), 0, 0, 'R');
        $this->SetTextColor(0, 0, 0);
    }
}

$pdf = new ProlinkPartPDF();
$w = [8, 46, 28, 28, 83, 18, 32, 30];
$hRow = 7.0;
$hHead = 8.0;
$labelsPdf = ['Id', 'Evenement', 'Nom', 'Prenom', 'Email', 'Tel', 'Inscription', 'Statut'];

$drawHero = static function (ProlinkPartPDF $pdf) use ($COL) {
    $W = 297.0;
    $pdf->SetY(0);
    $pdf->SetX(0);
    ...
    $pdf->SetX(14);
    $pdf->Cell(0, 7, prolink_pdf_s('Liste des participations'), 0, 1, 'L');
    $pdf->SetFont('Arial', '', 9);
...
$drawInfoBar = static function (ProlinkPartPDF $pdf, string $sort, string $dir, int $n) use ($COL) {
    $x = 12;
    $y = $pdf->GetY() + 2;
    $pdf->SetY($y);
    $ph = 7;
    ...
    $pdf->SetXY($x + 3, $y + 1.2);
    $pdf->SetFont('Arial', '', 7);
    $txt = prolink_pdf_s("Tri: {$sort}  " . strtoupper($dir) . "  |  {$n} participation(s)");
    $pdf->Cell($pw - 6, 5, $txt, 0, 0, 'L');
    $pdf->SetTextColor(0, 0, 0);
    $pdf->SetY($y + $ph + 2);
};

$drawTableHead = static function (ProlinkPartPDF $pdf, array $w, array $labelsPdf, int $hHead) use ($COL) {
    if ($pdf->GetX() < 1) {
        $pdf->SetX(12);
    }
    ...
    for ($i = 0, $c = min(count($w), count($labelsPdf)); $i < $c; $i++) {
...
foreach ($rows as $r) {
    ...
    $pdf->Cell($w[0], $hRow, (string) ($r['id_participation'] ?? ''), 1, 0, 'C', true);
    $pdf->SetFont('Arial', '', 6);
    $pdf->Cell($w[1], $hRow, prolink_pdf_trim((string) ($r['titre_event'] ?? ''), 32), 1, 0, 'L', true);
    $pdf->SetFont('Arial', '', 7);
    $pdf->Cell($w[2], $hRow, prolink_pdf_trim((string) ($r['nom'] ?? ''), 20), 1, 0, 'L', true);
    $pdf->Cell($w[3], $hRow, prolink_pdf_trim((string) ($r['prenom'] ?? ''), 20), 1, 0, 'L', true);
    $pdf->Cell($w[4], $hRow, prolink_pdf_trim((string) ($r['email'] ?? ''), 46), 1, 0, 'L', true);
    $pdf->SetFont('Arial', '', 7);
    $pdf->Cell($w[5], $hRow, (string) ($r['telephone'] ?? ''), 1, 0, 'C', true);
    $pdf->Cell($w[6], $hRow, prolink_pdf_trim((string) ($r['date_inscription'] ?? ''), 20), 1, 0, 'C', true);
    $pdf->SetFont('Arial', '', 6);
    $pdf->Cell($w[7], $hRow, prolink_pdf_trim((string) ($r['statut'] ?? ''), 18), 1, 0, 'C', true);
    $pdf->Ln();
    $pdf->SetTextColor(0, 0, 0);
}

$pdf->Output('D', 'prolink_participations_' . date('Y-m-d_His') . '.pdf');
exit;
